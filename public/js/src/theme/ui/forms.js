...
$(() => {
  ...
  $(document).addEventListener('DOMContentLoaded', function(){
    const fileInput = $('.hbspt-form input[type=file]');
    if(fileInput.length) {
      console.log('file upload exists');
    }
    const allowedFileTypes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];

    $(fileInput).addEventListener('change', function(){
      if(!allowedFileTypes.includes(fileInput.files[0].type)){
        alert('pdf or words docs check');
        fileInput.val() = '';
      }
    });
  });
  ...
  var blogCTAHiddenFields = setInterval(function(){
    if($('.hbspt-form').length){
      $('.hbspt-form').find('input[type=hidden]').each(function(){
        $(this).parents('fieldset').css('display','none');
      });
      if($('.hbspt-form input[type=file]').length) {
        console.log('blogCTAHiddenFields interval found the file upload');
      }
      clearInterval(blogCTAHiddenFields);
    }
  });
