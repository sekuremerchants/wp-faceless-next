...
$(() => {
  ...
  var hubspotFormsFields = setInterval(function(){
    if($('.hbspt-form').length){
      ...
      if($('.contacts-tab-form .hbspt-form input[type=file]').length) {
        $('.contacts-tab-form .hbspt-form input[type=file]').attr('accept', '.doc, .docx, application/pdf');
      }
  ...
  $(document).on('change', '.contacts-tab-form .hbspt-form input[type=file]', function() {
    ...
    if ($('div.hs_submit input[type=submit]').length) {
      ...
      $('div.hs_submit').each(function () {
        if (!$(this).parents('#footer').length && !$(this).parents('.popup').length) {
          ...
          if($(this).parents('.btn-custom-text').length){
            label = $(this).parents('.btn-custom-text').data('custom-text');
          }
  ...
  if($('.choices__item.choices__item--choice').length){
    console.log('choices exist');
    $('.choices__item.choices__item--choice').text().replace('&amp;', '&');
  } else {
